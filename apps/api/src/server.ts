...
export async function buildApp() {
  ...
  const allowedOriginPatterns = env.CORS_ORIGIN.split(",")
    .map((o) => o.trim())
    .filter(Boolean);
  // CORS_ORIGIN accepts literal origins (http://localhost:5173) and wildcard
  // patterns with `*` in the host label (https://*.thebrain.app, http://*.localhost:5173).
  // Wildcards are expanded to a regex for the @fastify/cors `origin` function; CSP
  // `connect-src` understands the same `*.host` syntax natively.
  const originRegexes = allowedOriginPatterns.map((pattern) => {
  ...
  await app.register(helmet, {
    contentSecurityPolicy: {
      directives: {
        ...
        connectSrc: ["'self'", ...allowedOriginPatterns],
        objectSrc: ["'none'"],
  ...
  await app.register(cors, {
    origin: (origin, cb) => {
      ...
      cb(
        null,
        originRegexes.some((re) => re.test(origin)),
      );
    },
    credentials: true,
  });
