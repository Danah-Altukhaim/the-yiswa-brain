...
// Anchored on word boundaries so substrings inside unrelated keys (like "end"
// inside "gender") don't accidentally flip a field into date-picker rendering.
const DATE_KEY_RE = /(^|_)(date|deadline|expires?|published|start|end)($|_)/i;
...
// Parse legacy multi_select values: may arrive as an array, newline-delimited
// string, or comma-delimited string. Returns a clean, de-duplicated array.
function parseMultiSelectValue(raw: unknown): string[] {
  const pushClean = (out: string[], seen: Set<string>, s: string) => {
    const cleaned = stripIcons(s)
      .replace(/^[-•·*\s]+|[\s,;.]+$/g, "")
      .trim();
    if (!cleaned) return;
  ...
  if (Array.isArray(raw)) {
    for (const v of raw) pushClean(out, seen, String(v ?? ""));
    return out;
  }
  if (raw == null) return out;
  const s = String(raw);
  if (!s.trim()) return out;
  // Split on newlines first to respect multi-line entries, then comma-split
  // only lines that look like a single flat list (no trailing words).
  const lines = s.split(/\r?\n/);
  for (const line of lines) {
    if (line.includes(",") && line.length <= 120) {
      for (const part of line.split(",")) pushClean(out, seen, part);
    } else {
      pushClean(out, seen, line);
    }
  }
  return out;
}
...
const AGENT_CONFIG_SLUGS = new Set(["flare_agent_config", "macro_agent_config"]);
...
export function ModulePage() {
  ...
  const sectionOptions = useMemo(() => {
    if (!AGENT_CONFIG_SLUGS.has(slug)) return [] as string[];
    const seen = new Set<string>();
    const ordered: string[] = [];
    for (const e of entries) {
      const v = (e.data as Record<string, unknown>).section;
          ...
          <p className="text-[13px] text-apple-secondary mt-0.5">
            {loading
              ...
              : query.trim()
                ...
                : AGENT_CONFIG_SLUGS.has(slug) && sectionOptions.length > 0
                  ? `${entries.length} ${entries.length === 1 ? "entry" : "entries"} across ${sectionOptions.length} ${sectionOptions.length === 1 ? "section" : "sections"}`
...
type Column = {
  key: string;
  label: string;
  shape: "id" | "bool" | "date" | "url" | "hours" | "bilingual" | "text" | "file";
  primary: boolean;
  enKey?: string | null;
  arKey?: string | null;
  dataKey?: string;
...
function pdfHref(filename: string): string {
  ...
  return `/pdfs/${encodeURIComponent(s)}`;
}
...
function deriveColumns(
  ...
  const defByKey: Record<string, FieldDef> = {};
  if (fieldDefinitions) {
    for (const f of fieldDefinitions) {
      if (f.localized) {
        defByKey[`${f.key}_en`] = f;
        defByKey[`${f.key}_ar`] = f;
      } else {
        defByKey[f.key] = f;
      }
    }
  ...
  for (const g of langGroups) {
    if (g.kind === "pair") {
      ...
      const def = defByKey[g.base] ?? (g.enKey ? defByKey[g.enKey] : undefined);
      cols.push({
        key: `pair:${g.base}`,
        label: def?.label ?? humanizeKey(g.base),
        shape: "bilingual",
        primary: isPrimary,
        enKey: g.enKey,
        arKey: g.arKey,
      ...
      const def = defByKey[g.key];
      let shape: Column["shape"] = "text";
      if (def?.type === "file") shape = "file";
      else if (samples.length > 0 && samples.every(isBoolish)) shape = "bool";
      ...
      const isPrimary = !primaryAssigned && (shape === "text" || NAME_RE.test(g.key));
      if (isPrimary) primaryAssigned = true;

      cols.push({
        key: `single:${g.key}`,
        label: def?.label ?? humanizeKey(g.key),
        shape,
        primary: isPrimary,
        dataKey: g.key,
      });
  ...
  if (cols.length > 0 && cols.length <= 4) {
    for (const c of cols) c.primary = true;
  }
    ...
    case "file":
      return (
        <a
          ...
          rel="noopener noreferrer"
          onClick={(e) => e.stopPropagation()}
          className="text-[13px] text-pair hover:underline truncate inline-block max-w-[260px]"
          title={s}
        >
          {s}
        </a>
...
function AgentConfigTable({
  ...
  const groups = useMemo(() => {
    ...
    const order: string[] = [];
    for (const e of entries) {
      const raw = (e.data as Record<string, unknown>).section;
      const section = (typeof raw === "string" && raw.trim()) || "Uncategorized";
      if (!map.has(section)) {
        map.set(section, []);
        order.push(section);
      }
    ...
    <div className="space-y-4">
      {groups.map(({ section, items }) => (
        <div key={section} className="card overflow-hidden">
          <div className="flex items-center justify-between gap-3 px-4 py-2.5 bg-[#FBFBFD] border-b border-apple-separator-light">
            <span className="text-[11px] uppercase tracking-[0.08em] font-semibold text-apple-text truncate">
              {section}
            </span>
            <span className="shrink-0 text-[11px] text-apple-tertiary tabular-nums">
              {items.length} {items.length === 1 ? "entry" : "entries"}
            </span>
          </div>
          <ul className="divide-y divide-apple-separator-light">
            {items.map((e) => {
              const d = e.data as Record<string, unknown>;
              const rawKey = stripIcons(String(d.key ?? "")).trim();
              const isBullet = BULLET_PREFIX_RE.test(rawKey);
              const cleanKey = rawKey.replace(BULLET_PREFIX_RE, "");
              const value = stripIcons(String(d.value ?? "")).trim();
              const valueIsAr = ARABIC_RE.test(value);
              const valueIsUrl = /^https?:\/\/|^[a-z0-9-]+\.[a-z]{2,}\//i.test(value);
              return (
                <li
                  key={e.id}
                  onClick={() => onEdit(e)}
                  className="grid grid-cols-[minmax(0,1fr)_auto] sm:grid-cols-[minmax(180px,260px)_minmax(0,1fr)_auto] items-start gap-x-4 gap-y-1 px-4 py-3 hover:bg-[#F9F9F9] cursor-pointer transition-colors"
                >
                  <div className="min-w-0 flex items-start gap-2">
                    {isBullet && (
                      <span
                        aria-hidden
                        className="text-apple-tertiary leading-5 select-none shrink-0 pt-px"
                      >
                        •
                      </span>
                    )}
                    <span
                      className="text-[13px] leading-5 font-medium text-apple-text truncate"
                      title={cleanKey}
                    >
                      {cleanKey || <span className="text-apple-tertiary">(unnamed)</span>}
                    </span>
                  </div>
                  <div className="min-w-0 col-span-2 sm:col-span-1 order-3 sm:order-none">
                    {value ? (
                      <div
                        dir={valueIsAr ? "rtl" : "ltr"}
                        title={value}
                        className={`text-[13px] leading-5 truncate ${
                          valueIsUrl ? "text-pair" : "text-apple-secondary"
                        } ${valueIsAr ? "font-arabic text-right" : "text-left"}`}
                      >
                        {value}
                      </div>
                    ) : (
                      <span className="text-[12px] italic text-apple-tertiary">
                        {isBullet ? "list item" : "header / no value"}
                      </span>
                    )}
                  </div>
                  <span
                    className={`badge ${badgeFor(e.status)} shrink-0 self-start sm:self-center`}
                  >
                    {e.status || "active"}
                  </span>
                </li>
...
function EntryTable({
  ...
  const cols = useMemo(
    () => deriveColumns(entries, langGroups, fieldDefinitions),
    [entries, langGroups, fieldDefinitions],
  );
...
function EditEntryModal({
  ...
  const isAgentConfig = AGENT_CONFIG_SLUGS.has(slug);
  const initialKeys = useMemo(() => {
    const keys = Object.keys(entry.data).filter((k) => !HIDDEN_EDIT_KEYS.has(k));
    if (fieldDefinitions && fieldDefinitions.length > 0) {
      const declared: string[] = [];
      for (const f of fieldDefinitions) {
        if (f.localized) {
          declared.push(`${f.key}_en`, `${f.key}_ar`);
        } else {
          declared.push(f.key);
        }
      }
      ...
      const extras = keys.filter((k) => !declaredSet.has(k));
      return orderKeys([...ordered, ...extras]);
    }
  ...
  const getHelp = useMemo(() => {
    const map: Record<string, string> = {};
    if (fieldDefinitions) {
      for (const f of fieldDefinitions) {
        if (!f.help) continue;
        if (f.localized) {
          map[`${f.key}_en`] = f.help;
          map[`${f.key}_ar`] = f.help;
        } else {
          map[f.key] = f.help;
        }
      }
  ...
  const multiSelectKeys = useMemo(() => {
    const s = new Set<string>();
    if (fieldDefinitions) {
      for (const f of fieldDefinitions) {
        if (f.type === "multi_select") s.add(f.key);
      }
    }
    return s;
  }, [fieldDefinitions]);
  ...
  const [draft, setDraft] = useState<Record<string, unknown>>(() =>
    Object.fromEntries(
      initialKeys.map((k) => {
        ...
        if (multiSelectKeys.has(k)) {
          return [k, parseMultiSelectValue(rawVal)];
        }
      ...
      >
        <div className="flex items-center justify-between px-4 sm:px-5 py-3 sm:py-3.5 border-b border-apple-separator-light">
          <div className="min-w-0 flex-1 mr-3">
            {isAgentConfig && !isNew ? (
              (() => {
                const sec = String((entry.data as Record<string, unknown>).section ?? "").trim();
                const k = stripIcons(String((entry.data as Record<string, unknown>).key ?? ""))
                  .replace(BULLET_PREFIX_RE, "")
                  .trim();
                return (
                  <>
                    {sec && (
                      <div className="text-[10px] uppercase tracking-[0.08em] font-semibold text-apple-tertiary mb-0.5 truncate">
                        {sec}
                      </div>
                    )}
                    <div className="text-[15px] font-semibold text-apple-text truncate">
                      {k || "Edit entry"}
                    </div>
                  </>
                );
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
          ...
          {(() => {
            ...
            const isShortField = (key: string) => {
              ...
              if (multiSelectKeys.has(key)) return false;
            ...
            const renderField = (key: string) => {
                ...
                return (
                  <div key={key}>
                    ...
                    <p className="text-[11px] text-apple-tertiary mt-1">
                      Groups this entry under a heading on the Agent Config page.
                    </p>
                  </div>
                );
              }
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {val.trim() && (
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="shrink-0 text-[12px] text-pair hover:underline inline-flex items-center gap-1 px-2 py-1"
                          title={`Open ${val}`}
                        >
              ...
              const val = String(rawVal);
              const forceLong = isAgentConfig && key === "value";
              const isLong = forceLong || (!opts && (val.length > 80 || val.includes("\n")));
              const isAr = ARABIC_RE.test(val);
              const labelText =
                isAgentConfig && key === "key"
                  ? "Label"
                  : isAgentConfig && key === "value"
                    ? "Content"
                    : getLabel(key);
              const placeholder =
                isAgentConfig && key === "key"
                  ? "e.g. Hotline, Refund Policy, 1st Priority"
                  : isAgentConfig && key === "value"
                    ? "What the agent should know or say. Leave empty for headers."
                    : undefined;
              ...
              return (
                <div key={key}>
                  ...
                  {opts ? (
                    ...
                    <div className="relative">
                      <input
                        type="text"
                        dir="ltr"
                        className="input-apple pr-8 text-left"
                        value={val}
                        onChange={(ev) => setDraft({ ...draft, [key]: ev.target.value })}
                      />
                      <span className="pointer-events-none absolute inset-y-0 right-3 flex items-center text-apple-tertiary">
                        %
                      </span>
                    </div>
                  ) : (
                  ...
                  {getHelp(key) && (
                    <p className="text-[11px] text-apple-tertiary mt-1">{getHelp(key)}</p>
                  )}
                </div>
              );
            };
            ...
            while (i < initialKeys.length) {
              ...
              // Select/multi_select fields declared in the module schema win
              // over date heuristics, so `gender: "select"` doesn't get pulled
              // into the date-picker branch by a fuzzy regex match.
              const isShort = !isHoursField && isShortField(k);
              const isDate =
                !isHoursField && !isShort && (DATE_KEY_RE.test(k) || ISO_DATE_RE.test(vStr));
...
function MultiSelectChips({
  ...
  const suggestions = useMemo(() => {
    ...
    return options
      .filter((o) => {
        if (selectedLower.has(o.toLowerCase())) return false;
        if (!q) return true;
        return o.toLowerCase().includes(q);
      })
      .slice(0, 20);
  }, [draft, options, selectedLower]);
  ...
  return (
    <div ref={containerRef} className="relative">
      <div
        className="input-apple flex flex-wrap gap-1.5 min-h-[38px] py-1.5 cursor-text"
        onClick={() => setOpen(true)}
      >
        {value.map((v, i) => (
          <span
            key={`${v}-${i}`}
            className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-[#E8F0FE] text-[12px] text-[#1A73E8]"
          >
            {v}
            <button
              type="button"
              ...
              className="text-[#1A73E8]/70 hover:text-apple-red"
              aria-label={`Remove ${v}`}
            >
              <Icon name="close" size={10} />
            </button>
        ...
        <input
          ...
          className="flex-1 min-w-[120px] bg-transparent outline-none text-[14px]"
          placeholder={value.length ? "Add another…" : "Pick from list or type to add"}
        />
      </div>
      {open && (suggestions.length > 0 || draft.trim()) && (
        <div className="absolute z-10 mt-1 w-full max-h-60 overflow-y-auto rounded-apple border border-apple-separator-light bg-white shadow-apple">
          {suggestions.map((o) => (
            <button
              key={o}
              type="button"
              onClick={() => add(o)}
              className="block w-full text-left px-3 py-2 text-[13px] text-apple-text hover:bg-[#F2F2F5]"
            >
              {o}
            </button>
          ))}
          {draft.trim() && !options.some((o) => o.toLowerCase() === draft.trim().toLowerCase()) && (
            <button
              type="button"
              onClick={() => add(draft)}
              className="block w-full text-left px-3 py-2 text-[13px] text-pair hover:bg-[#F2F2F5] border-t border-apple-separator-light"
            >
              + Add "{draft.trim()}"
            </button>
          )}
